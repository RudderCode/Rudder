...
// rudder-spec: REQ-010
test('finished runs report spec-backed test and Rudder question counts', () => {
  ...
  assert.equal(result.spec_backed_test_count, 5);
...
// rudder-spec: REQ-008
test('spec creation telemetry exposes no spec lifecycle data', () => {
  const result = properties('spec-created', {
    host: 'codex',
    repository: 'github.com/private/raw-repository',
    branch: 'raw-branch',
    runId: 'raw-run-id',
    originType: 'repository',
    sourceRelativePath: 'RAW-SOURCE-PATH',
    specPath: 'RAW-LOCAL-SPEC-PATH',
    contents: 'RAW-SPEC-CONTENTS',
  });
  ...
  assert.doesNotMatch(
    JSON.stringify(result),
    /raw-repository|raw-branch|raw-run-id|RAW-SOURCE-PATH|RAW-LOCAL-SPEC-PATH|RAW-SPEC-CONTENTS|originType/
  );
});
